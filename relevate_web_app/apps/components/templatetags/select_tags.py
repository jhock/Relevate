...
def text_input(parser, token):
  ...
  if editable == 'False':
    input_props.append('readonly')

  return Select(children, label, input_id, alt_input, input_props, options)


class Select(Node):
  def __init__(self, children, label, input_id, alt_input, input_props, options):
