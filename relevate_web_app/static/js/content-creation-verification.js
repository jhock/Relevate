	...
	$(".submit-button").on('click', function()	{
		...
		var flags = {
			...
			blurb: {
				...
				message: "Short description must have content between 0 and 300 characters"
			},
			references: {
			...
			content_file: {
				...
				message: "You must upload a valid PDF, Doc, Mp4, Webm, or Docx file."
			}
		};
		...
		//If user decides to enter a file (it is not required.)
		if ($("#id_content_file").val().length != 0 ) {
            if (!flags.content_file.done) {
                good = false;
                errorMsg += "<li>" + flags.content_file.message + "</li>";
            }
        }
...
function fileIsPDForDoc(){
	/**
	 * Checks if the upload image is a valid image.
	 * It first checks if there exist an image, then checks if user uploaded an image.
	 * @type {string[]}
     */
	var listOfExtensions = ['pdf', 'doc', 'docx', 'mp4', 'webm'];
	var contentVal = $("#id_content_file").val();
	var ext = "";
	try{
		ext = contentVal.substr(contentVal.lastIndexOf('.')+1);
	}catch (err){
